...
export async function postComment(comment:Comment) : Promise<Message| ErrorMessage>{
    const resp = await fetch(`${import.meta.env.VITE_SERVER_URL}/comments`, {method:"POST", headers:{"Content-Type":"application/json"}, body:JSON.stringify(comment)});
    if(!resp.ok)
...
export async function deleteComment(id: string) : Promise<Message | ErrorMessage>{
    const resp = await fetch(`${import.meta.env.VITE_SERVER_URL}/comments/${id}`, {method:"DELETE", headers:{"Content-Type":"application/json"}});
    if(!resp.ok)
